    ...
    def handle_limit_order(self, order): 
        ...
        if order.side == OrderSide.BUY:
            ...
            if len(self.ask_book) != 0:
                # if ask book is not empty
                for ask_order in self.ask_book:
                    # if order is limit order and the ask price is greater then the current order price then those two limit orders / a limit and a market could be traded
                    if (ask_order.type == OrderType.LIMIT and ask_order.price >= order.price) or ask_order.type == OrderType.MARKET:                        
                        # if ask quantity less and current order quantity
        ...
        elif order.side == OrderSide.SELL: #for sell side/action
            order_fully_traded = False
            if len(self.bid_book) != 0: #if bid order book is not empty
                for bid_order in self.bid_book:
                    #if its limit order and price greater thn order price  or a martlet order then
                    #the trade is between two limited orders / a limited order and a market order
                    if (bid_order.type == OrderType.LIMIT and bid_order.price > order.price) or bid_order.type == OrderType.MARKET:
                        if bid_order.quantity < order.quantity: # if bid quantity less and current order quantity
                            order.quantity -= bid_order.quantity; #update current order
